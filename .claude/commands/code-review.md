...
- ✅ pre-commit hooks (Ruff, Black) pass.
- 🔒 No security vulnerabilities (e.g., from safety scan).
- ⚡ API response time < 200ms (for the initial 202 response).
- 📊 pytest-cov compliance.
...
loan-prequal-system/
├── services/
│   ├── prequal-api/      # FastAPI service [cite: 49]
│   │   ├── app/
│   │   ├── tests/
│   │   ├── Dockerfile
│   │   └── pyproject.toml
│   ├── credit-service/   # Kafka consumer service [cite: 58]
│   │   ├── app/
│   │   ├── tests/
│   │   ├── Dockerfile
│   │   └── pyproject.toml
│   └── decision-service/ # Kafka consumer service [cite: 74]
│       ├── app/
│       ├── tests/
│       ├── Dockerfile
│       └── pyproject.toml
